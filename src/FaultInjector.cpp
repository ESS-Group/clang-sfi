...
bool FaultInjector::checkStmt(const Decl *stmt, std::string binding,
                              ASTContext &Context) {
    return false;
}
...
void FaultInjector::matchAST(ASTContext &Context) { Matcher.matchAST(Context); }
...
std::string FaultInjector::stmtToString(const Stmt *stmt,
...
void FaultInjector::dumpStmt(const Decl *decl) { decl->dump(); }

std::string FaultInjector::stmtToString(const Decl *decl,
                                        const LangOptions &langOpts) {
    std::string statement;
    raw_string_ostream stream(statement);
...
std::string FaultInjector::getEditedString(Rewriter &rewrite,
                                           ASTContext &Context) {
    return rewriteBufferToString(
...
std::string
FaultInjector::sourceLocationToString(SourceLocation loc,
                                      const SourceManager &sourceManager) {
    return loc.printToString(sourceManager);
}

std::string
FaultInjector::sourceRangeToString(SourceRange range,
                                   const SourceManager &sourceManager) {
...
std::string
FaultInjector::sourceRangeToString(const Stmt *stmt,
                                   const SourceManager &sourceManager) {
    return sourceRangeToString(stmt->getSourceRange(), sourceManager);
}

std::string
FaultInjector::sourceRangeToString(const Decl *decl,
                                   const SourceManager &sourceManager) {
    return sourceRangeToString(decl->getSourceRange(), sourceManager);
}

void FaultInjector::printStep(StmtBinding current,
                              const SourceManager &sourceManager,
                              const LangOptions &langOpts, int i, int size) {
    cout << "injecting '" << toString() << "' [" << i + 1 << "/" << size << "]"
         << endl;
    if (current.isStmt) {
        if (current.isList) {
            // cout << sourceRangeToString(current.stmt,sourceManager)<<endl;
            cout << "List with " << current.stmtlist.size() << " Statements"
                 << endl;
        } else {
            cout << sourceRangeToString(current.stmt, sourceManager) << endl;
...
void FaultInjector::printStep(StmtBinding current,
                              const SourceManager &sourceManager, int i,
                              int size) {
    cout << "injecting '" << toString() << "' [" << i + 1 << "/" << size << "]"
         << endl;
    /*if(current.isStmt){
...
void FaultInjector::setVerbose(bool v) { verbose = v; }

void FaultInjector::setDirectory(std::string directory) { dir = directory; }
void FaultInjector::inject(std::vector<StmtBinding> target,
                           ASTContext &Context) {
    int i = 0;
    ...
    for (StmtBinding current : target) {
        if (verbose)
            printStep(current, Context.getSourceManager(),
                      Context.getLangOpts(), i++, target.size());
        else
            i++;
        // else
...
void FaultInjector::writeDown(std::string data, int i) {
    std::string name = (dir.compare("") ? dir + "/" : "") + toString() + "_" +
                       std::to_string(i);
