...
std::string MIAInjector::toString() { return "MIA"; };
std::string MIAInjector::inject(StmtBinding current, ASTContext &Context) {
    ...
    SourceRange range(ifS->getLocStart(),
                      ifS->getThen()->getLocStart().getLocWithOffset(-1));
...
bool MIAInjector::checkStmt(const Stmt *stmt, std::string binding,
...
std::string SMIAInjector::toString() { return "SMIA"; };
bool SMIAInjector::checkStmt(const Stmt *stmt, std::string binding,
                             ASTContext &Context) {
    const IfStmt *ifS = (IfStmt *)(stmt);
