        ...
        varDecl( // variable declaration
            allOf(
                hasInitializer( // variable declaration that has en initializer
                    unless(anyOf( // assure that initializer is an value
                        callExpr(), hasDescendant(callExpr()), cxxNewExpr(),
                        hasDescendant(cxxNewExpr()), binaryOperator(),
                        hasDescendant(binaryOperator()), unaryOperator(),
                        hasDescendant(unaryOperator()), cxxConstructExpr(),
                        hasDescendant(cxxConstructExpr()), declRefExpr(),
                        hasDescendant(declRefExpr()), memberExpr(),
                    ...
                    ,
                hasDeclContext(
                    functionDecl()) // declaration in function context (local)
                )

                )
            .bind("variable"),
        createStmtHandler("variable"));
}
...
bool MVIVInjector::checkStmt(const Decl *decl, std::string binding,
    ...
    if (const ForStmt *forstmt = getParentOfType<ForStmt>(declstmt, Context, 3))
        return isParentOf(forstmt->getBody(), declstmt) &&
               !((const VarDecl *)decl)->isStaticLocal() && C2(decl, Context);
    else
