...
std::string MIFSInjector::toString() { return "MIFS"; };
...
bool MIFSInjector::checkStmt(const Stmt *stmt, std::string binding,
    ...
    return /*C8(ifS) && */ C2(
        stmt,
        Context); // also if the statement is the only statement in the block
    //} else return false;
}

bool SMIFSInjector::checkStmt(const Stmt *stmt, std::string binding,
                              ASTContext &Context) {
    const IfStmt *ifS = (IfStmt *)(stmt);
...
std::string SMIFSInjector::toString() { return "SMIFS"; };
