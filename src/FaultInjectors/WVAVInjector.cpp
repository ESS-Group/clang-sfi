...
WVAVInjector::WVAVInjector(bool alsoOverwritten) {
                ...
                hasLHS(               // left side of assignment
                    allOf(
                        ...
                        anyOf(
                            ...
                            ignoringParenCasts(
                                ignoringImplicit( // assignment to one time
                                                  // dereferred local pointer
                                    unaryOperator(allOf(
                                        hasOperatorName("*"),
                            ...
                            ignoringParenCasts(ignoringImplicit( // assignment
                                                                 // to one time
                                                                 // dereferred
                                                                 // pointer,
                                                                 // which is
    ...
    if (alsoOverwritten) { // overwritten assignmentoperator call, rest like
                           // above
        Matcher.addMatcher(
            cxxOperatorCallExpr(
                allOf(
...
bool WVAVInjector::checkStmt(const Stmt *stmt, std::string binding,
                             ASTContext &Context) {
    // if OVERWRITTENASSIGNMENTOPERATORISASSIGNEMENT
...
std::string WVAVInjector::inject(StmtBinding current, ASTContext &Context) {
    ...
    if (current.binding.compare("overwritten") == 0) {
        val =
            (Expr *)(&*((const CXXOperatorCallExpr *)current.stmt)->getArg(1));
    } else
...
std::string OWVAVInjector::toString() { return "OWVAV"; };
