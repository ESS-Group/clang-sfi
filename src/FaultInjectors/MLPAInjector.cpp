...
getStmtLists(const CompoundStmt *block, ASTContext &Context,
    ...
    int index = -1;
    for (Stmt::child_iterator i = cast_away_const(block->child_begin()),
                              e = cast_away_const(block->child_end());
         i != e; ++i) {
        if (*i == NULL) {
            ...
            if ((noDeclStmt || !isa<DeclStmt>(*i)) && !isa<IfStmt>(*i) &&
                !isa<ForStmt>(*i) && !isa<WhileStmt>(*i) && !isa<DoStmt>(*i) &&
                !isa<SwitchStmt>(*i) &&
                (!returnIsAJump || !isa<ReturnStmt>(*i)) &&
    ...
    for (; rit != ret.rend(); /*++rit*/) {
        ...
        std::vector<const DeclStmt *> declstmts =
            getStmtsOfType<DeclStmt>(list);

        std::vector<const DeclStmt *> notPossible;

        for (const DeclStmt *declstmt :
             declstmts) { // calculate if declstatements in list cannot be
            ...
            for (auto decl : declstmt->decls()) {
                ...
                const DeclRefExpr *latest =
                    getLatestRef(block, (const VarDecl *)decl);
            ...
            if (ref.size() /*ref!=NULL*/) {
                ...
                for (const DeclRefExpr *reference : ref) {
                    ...
                    if (reference != NULL &&
                        list.back()->getLocEnd() < reference->getLocStart() &&
                        std::find(list.begin(), list.end(), declstmt) !=
                            list.end()) {

                        const DeclStmt *statement =
                            (const DeclStmt *)declstmt; // declStmt;
        ...
        if (notPossible.size() != 0) {
            ...
            std::sort(notPossible.begin(), notPossible.end(),
                      _comparefunc<DeclStmt>);

            int ritpos = std::distance(ret.rbegin(), rit) + changed.size();

            ret.insert(ret.end(), changed.begin(),
                       changed.end()); // invalidates iterator
            rit = ret.rbegin();
...
bool isMLPAListPossible(std::vector<const Stmt *> stmtlist,
                        const CompoundStmt *block) {
    std::vector<const DeclStmt *> declstmts =
        getStmtsOfType<DeclStmt>(stmtlist);

    std::vector<const DeclStmt *> notPossible;

    for (const DeclStmt *declstmt :
         declstmts) { // calculate if declstatements in list cannot be removed,
        ...
        for (const DeclRefExpr *reference : ref) {
            if (reference != NULL &&
                stmtlist.back()->getLocEnd() < reference->getLocStart() &&
                std::find(stmtlist.begin(), stmtlist.end(), declstmt) !=
                    stmtlist.end()) {
                const DeclStmt *statement =
                    (const DeclStmt *)declstmt; // declStmt;
                notPossible.push_back(statement);
...
std::vector<std::vector<const Stmt *>>
getMLPAListOfSize(std::vector<const Stmt *> stmtlist, int size,
                  const CompoundStmt *block) {
    std::vector<std::vector<const Stmt *>> ret;
    int listsize = stmtlist.size();
    for (int begin = 0; begin + size <= listsize; begin++) {
        std::vector<const Stmt *> list(stmtlist.begin() + begin,
                                       stmtlist.begin() + begin + size);
...
MLPAInjector::MLPAInjector() {
    Matcher.addMatcher(compoundStmt(allOf(unless(hasParent(declStmt())),
                                          unless(hasParent(switchStmt()))))
                           .bind("compoundStmt"),
                       createStmtHandler("compoundStmt"));
}

std::string MLPAInjector::toString() { return "MLPA"; };
...
bool MLPAInjector::checkStmt(const Stmt *stmt, std::string binding,
                             ASTContext &Context) {

    const CompoundStmt *compoundStmt = (const CompoundStmt *)stmt;

    std::vector<std::vector<const Stmt *>> stmtlists =
        getStmtLists(compoundStmt, Context);
    // int s = stmt->size();
    /*
        ...
        if (it.size() >= 2) {
            ...
            for (; size >= 2; size--) {
                ...
                std::vector<std::vector<const Stmt *>> injectionpoints =
                    getMLPAListOfSize(it, size,
                                      compoundStmt); // create SubList of size

                for (std::vector<const Stmt *> injectionpoint :
                     injectionpoints) {
...
std::string SMLPAInjector::toString() { return "SMLPA"; };

bool SMLPAInjector::checkStmt(const Stmt *stmt, std::string binding,
                              ASTContext &Context) {
    ...
    for (std::vector<const Stmt *> it : stmtlists) {
        ...
        if (it.size() >= 2) {
            ...
            for (; size >= 2; size--) {
                std::vector<std::vector<const Stmt *>> injectionpoints =
                    getMLPAListOfSize(it, size, compoundStmt);
                for (std::vector<const Stmt *> injectionpoint :
                     injectionpoints) {
                    nodeCallback(binding, injectionpoint);
                }
