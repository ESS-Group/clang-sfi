...
std::string WPFVInjector::toString() { return "WPFV"; };
...
std::string WPFVInjector::inject(StmtBinding current, ASTContext &Context) {
    ...
    const FunctionDecl *fkt =
        (const FunctionDecl *)declContext->getNonClosureAncestor();
    ...
    for (int i = 0; i < paramCount; i++) {
        ...
        if (param != arg &&
            param->getType().getNonReferenceType().getDesugaredType(Context) ==
                arg->getType().getNonReferenceType().getDesugaredType(
                    Context) &&
    ...
    if (!isParam || localVariables.size() == 0) {
        for (DeclContext::decl_iterator it = declContext->decls_begin(),
            ...
            if (isVisible(vardecl, stmt, Context) && vardecl != arg &&
                arg->getType().getNonReferenceType().getDesugaredType(
                    Context) ==
                    vardecl->getType().getNonReferenceType().getDesugaredType(
                        Context)) {
                if (!isParam)
...
bool WPFVInjector::checkStmt(const Stmt *stmt, std::string binding,
                             ASTContext &Context) {
    for (auto i : getArgumentsOfType<DeclRefExpr>((const CallExpr *)stmt)) {
        const VarDecl *arg = (VarDecl *)i->getDecl();
        const DeclContext *declContext = arg->getDeclContext();
        int varcount = 0;
        const FunctionDecl *fkt = getParentFunctionDecl(
            getParentOfType<DeclStmt>(arg, Context), Context);
        if (fkt == NULL) {
            const FunctionDecl *_fkt =
                (const FunctionDecl *)declContext->getNonClosureAncestor();
            if (_fkt != NULL) {
                ...
                for (int i = 0; i < paramCount; i++) {
                    ...
                    if (param != arg &&
                        param->getType().getNonReferenceType().getDesugaredType(
                            Context) ==
                            arg->getType()
                                .getNonReferenceType()
                ...
                for (DeclContext::decl_iterator it = _fkt->decls_begin(),
                                                e = _fkt->decls_end();
                     it != e; ++it) {
                    const VarDecl *vardecl = (const VarDecl *)*it;
                    if (vardecl != arg &&
            ...
            for (DeclContext::decl_iterator it = fkt->decls_begin(),
                                            e = fkt->decls_end();
                 it != e; ++it) {
                const VarDecl *vardecl = (const VarDecl *)*it;
                if (vardecl != arg &&
