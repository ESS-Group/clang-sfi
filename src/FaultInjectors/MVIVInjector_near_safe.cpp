...
bool MVIVInjectorSAFE::checkStmt(const Decl *decl, std::string binding,
    ...
    if (binding.compare("notInitialized") == 0 && isa<VarDecl>(decl)) {
        std::vector<const BinaryOperator *> list = getChildForFindInitForVar(
            getParentCompoundStmt(decl, Context), (const VarDecl *)decl);
        ...
        if (C2(decl, Context)) {
            ...
            const DeclStmt *declsstmt =
                getParentOfType<DeclStmt>(decl, Context, 2);
            const ForStmt *stmt =
                getParentOfType<ForStmt>(declsstmt, Context, 5);

            return stmt != NULL &&
                   (isParentOf(stmt->getInit(), decl, Context) ||
                        stmt->getInit(),
                    declsstmt);
        }
    }
