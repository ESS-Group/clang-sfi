...
WAEPInjector::WAEPInjector() {
    Matcher.addMatcher(
        callExpr(
            allOf(unless(anyOf(cudaKernelCallExpr(), cxxOperatorCallExpr(),
                               userDefinedLiteral())),
                  hasDescendant(expr(anyOf(binaryOperator(),
                                           hasDescendant(binaryOperator()))))))
            .bind("functionCall"),
        createStmtHandler("functionCall"));
    // Matcher.addMatcher(callExpr(unless(anyOf(hasParent(varDecl(isDefinition())),
    // hasParent(returnStmt()), cxxOperatorCallExpr(), hasParent(callExpr()),
    // hasParent(expr())))).bind("FunctionCall"),
    // createStmtHandler("FunctionCall"));
    // Matcher.addMatcher(callExpr().bind("FunctionCall"),
    // createStmtHandler("FunctionCall"));
}

std::string WAEPInjector::toString() { return "WAEP"; };
...
std::string WAEPInjector::inject(StmtBinding current, ASTContext &Context) {
    ...
    start =
        ((const BinaryOperator *)current.stmt)
            ->getOperatorLoc(); //.getLocWithOffset(-1);//.getLocWithOffset(1);
...
bool WAEPInjector::checkStmt(const Stmt *stmt, std::string binding,
                             ASTContext &Context) { // no else
    if (hasChildOfType<BinaryOperator>(stmt)) {
        std::vector<const BinaryOperator *> arguments =
            getChildrenFlat<BinaryOperator>(stmt);
        for (const BinaryOperator *op : arguments) {
            // int code = op->getOpcode();
            if (isArithmetic(op)) {
                const BinaryOperator *rightest =
                    getBinaryOperatorWithRightedtRHS(op);
