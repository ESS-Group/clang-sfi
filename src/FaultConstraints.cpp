...
bool isaJumpStmt(const Stmt* stmt, bool returnIsAJump = true){
    if(stmt==NULL)
        ...
        return true;
    else if(isa<ContinueStmt>(stmt) || isa<BreakStmt>(stmt) || isa<CXXThrowExpr>(stmt)|| isa<CXXTryStmt>(stmt)|| isa<GotoStmt>(stmt))
        return true;
...
bool C9(const clang::Stmt::const_child_iterator &begin, const clang::Stmt::const_child_iterator &end, ASTContext* Context = NULL, bool returnIsAJump = RETURNISAJUMP, int maxNum = MAXSTATEMENTNUMFORCONSTRAINT, bool noDeclStmt = DONOTDELETEDECLSTMTINCONSTRAINT){
...
bool C9(const Stmt *stmt, ASTContext* Context = NULL, bool returnIsAJump = RETURNISAJUMP, int maxNum = MAXSTATEMENTNUMFORCONSTRAINT, bool noDeclStmt = DONOTDELETEDECLSTMTINCONSTRAINT){
...
CaseChilds getCaseChilds(const SwitchCase* sc, ASTContext &Context){
    ...
    if(switchStmt !=NULL){
        ...
        if(body == NULL)
            ...
            bool started = false;
            for(Stmt::child_iterator i = cast_away_const(body->child_begin()), e = cast_away_const(body->child_end());i!=e;++i){
                if(*i == NULL){
                    break;
                    ...
                    while(tcase!=NULL && tcase!=begin && tcase->getSubStmt()!=NULL && isa<SwitchCase>(tcase->getSubStmt()))
            ...
            if(stmt!=NULL && !isa<SwitchCase>(stmt))//otherwise case 1:case 2:break; would end in "case 2" being seen as child
                ret.stmts.push_back(stmt);
        }
